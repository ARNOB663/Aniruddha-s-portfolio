...
const leadershipData = [
    { role: '[ROLE]', organization: '[ORGANIZATION]', description: '[Description of your role and achievements in this organization.]' },
    { role: '[ROLE]', organization: '[ORGANIZATION]', description: '[Description of your role and achievements in this organization.]' },
    { role: '[ROLE]', organization: '[ORGANIZATION]', description: '[Description of your role and achievements in this organization.]' },
];
        ...
        >
            <div className="max-w-7xl mx-auto">
                    ...
                    <div className="lg:col-span-3 space-y-12">
                        ...
                        <motion.div variants={itemVariants}>
                            ...
                            <div className="relative border-l border-neutral-800 pl-8 space-y-8"> {/* CHANGED: border-neutral-800 */}
                                ...
                                <div>
                                    <p className="font-mono text-xs text-white">2023 - PRESENT</p> {/* CHANGED: text-white */}
                                    <h5 className="font-sans text-lg font-bold mt-1 text-white">BSc in Computer Science & Engineering</h5> {/* CHANGED: text-white */}
                                    <p className="text-neutral-400">East Delta University</p> {/* CHANGED: text-neutral-400 */}
                                    <p className="text-sm text-neutral-500 mt-2">Focusing on Backend Systems, Distributed Architecture, and Artificial Intelligence.</p> {/* CHANGED: text-neutral-500 */}
                                </div>
                            </div>
                        </motion.div>
                    </div>
                    ...
                    <div className="lg:col-span-2 space-y-12 bg-neutral-900/50 border border-neutral-800 p-8"> {/* CHANGED: bg and border */}
                        ...
                        <motion.div variants={itemVariants}>
                            ...
                            <div className="grid grid-cols-2 gap-x-6 gap-y-4 text-center border-t border-b border-neutral-800 py-4"> {/* CHANGED: border */}
                                <div><div className="font-sans text-3xl font-bold text-white">XX+</div><div className="font-mono text-xs text-neutral-500 mt-1">[METRIC_1]</div></div> {/* CHANGED: text color */}
                                <div><div className="font-sans text-3xl font-bold text-white">XX</div><div className="font-mono text-xs text-neutral-500 mt-1">[METRIC_2]</div></div> {/* CHANGED: text color */}
                                <div><div className="font-sans text-3xl font-bold text-white">XX+</div><div className="font-mono text-xs text-neutral-500 mt-1">[METRIC_3]</div></div> {/* CHANGED: text color */}
                                <div><div className="font-sans text-3xl font-bold text-white">XX+</div><div className="font-mono text-xs text-neutral-500 mt-1">[METRIC_4]</div></div> {/* CHANGED: text color */}
                            </div>
                        </motion.div>
                    </div>
                </div>
                ...
                <div className="mb-24">
                    ...
                    <motion.div className="grid grid-cols-1 md:grid-cols-3 gap-px bg-neutral-800 border border-neutral-800" variants={itemVariants}> {/* CHANGED: bg and border */}
                        {leadershipData.map((item, index) => (
                            <LeadershipCard key={`${item.organization}-${index}`} {...item} />
                        ))}
                    </motion.div>
                </div>
                ...
                <QuoteBlock variants={itemVariants}>
                    "[A SHORT QUOTE OR PHILOSOPHY THAT REPRESENTS YOUR CORE BELIEFS OR APPROACH TO WORK.]"
                </QuoteBlock>
            </div>
        </motion.section>
    );
