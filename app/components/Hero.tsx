...
const Hero = () => {
  ...
  useEffect(() => {
    const updateTime = () => {
      const now = new Date();
      const timeString = now.toLocaleTimeString('en-US', { timeZone: 'UTC', hour12: false, hour: '2-digit', minute: '2-digit', second: '2-digit' });
      setTime(`UTC ${timeString}`);
    };
    updateTime();
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-center">
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="space-y-4">
              <motion.h1 variants={itemVariants} className="font-sans text-5xl sm:text-6xl md:text-8xl lg:text-9xl font-extrabold leading-none text-black">
                ANIRUDDHA<br />DEV NATH
              </motion.h1>
              <motion.div variants={itemVariants} className="h-12 overflow-hidden">
                <div className="animate-text-slide">
                  <p className="font-mono text-xl md:text-2xl lg:text-3xl h-12 flex items-center text-gray-700">{'// CSE UNDERGRAD'}</p>
                  <p className="font-mono text-xl md:text-2xl lg:text-3xl h-12 flex items-center text-gray-700">{'// BACKEND DEVELOPER'}</p>
                  <p className="font-mono text-xl md:text-2xl lg:text-3xl h-12 flex items-center text-gray-700">{'// AI ENTHUSIAST'}</p>
                  <p className="font-mono text-xl md:text-2xl lg:text-3xl h-12 flex items-center text-gray-700">{'// CSE UNDERGRAD'}</p>
                </div>
              </motion.div>
            </div>
            ...
            <motion.p variants={itemVariants} className="max-w-xl text-gray-600 leading-relaxed text-base lg:text-lg">
              CSE undergrad and backend developer with a deep passion for artificial intelligence.
              Dedicated to building robust systems and exploring the frontiers of intelligent computing.
            </motion.p>

            {/* +++ REFACTORED: Using the new Button component for cleaner code +++ */}
            <motion.div variants={itemVariants} className="flex flex-wrap gap-4">
              <Button href="https://your-resume-link.com" isExternal>
                [ CHECK RESUME ]
              </Button>
              <Button href="mailto:your.email@example.com" variant="secondary" isExternal>
                [ INITIATE_CONTACT ]
              </Button>
            </motion.div>
          </div>
          ...
          <motion.div className="hidden lg:flex flex-col items-start gap-4 self-center" variants={rightClusterVariants}>
            ...
            <div className='flex items-center gap-1.5'>
              <p className="font-mono text-xs text-gray-400 tracking-widest">[//GEO_SIG: 0.0°N, 0.0°E]</p>
